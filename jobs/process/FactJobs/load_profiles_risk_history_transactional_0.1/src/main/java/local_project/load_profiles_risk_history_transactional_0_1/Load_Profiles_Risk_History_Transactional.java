...
package local_project.load_profiles_risk_history_transactional_0_1;
...
/**
 * Job: Load_Profiles_Risk_History_Transactional Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 7.0.1.20180411_1414
 * @status 
 */
public class Load_Profiles_Risk_History_Transactional implements TalendJob {
...
public String getCassandra_Username(){
	...
	private final String jobName = "Load_Profiles_Risk_History_Transactional";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Load_Profiles_Risk_History_Transactional.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Load_Profiles_Risk_History_Transactional.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row_Implicit_Context_RegexStruct implements routines.system.IPersistableRow<row_Implicit_Context_RegexStruct> {
    final static byte[] commonByteArrayLock_LOCAL_PROJECT_Load_Profiles_Risk_History_Transactional = new byte[0];
    static byte[] commonByteArray_LOCAL_PROJECT_Load_Profiles_Risk_History_Transactional = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_LOCAL_PROJECT_Load_Profiles_Risk_History_Transactional.length) {
				if(length < 1024 && commonByteArray_LOCAL_PROJECT_Load_Profiles_Risk_History_Transactional.length == 0) {
   					commonByteArray_LOCAL_PROJECT_Load_Profiles_Risk_History_Transactional = new byte[1024];
				} else {
   					commonByteArray_LOCAL_PROJECT_Load_Profiles_Risk_History_Transactional = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_LOCAL_PROJECT_Load_Profiles_Risk_History_Transactional, 0, length);
			strReturn = new String(commonByteArray_LOCAL_PROJECT_Load_Profiles_Risk_History_Transactional, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_LOCAL_PROJECT_Load_Profiles_Risk_History_Transactional) {
...
globalMap.put("JobID", 90);
globalMap.put("TableName", "Hitorical tables");
...
org.talend.components.jdbc.tjdbcrow.TJDBCRowProperties props_tDBRow_5 =
        (org.talend.components.jdbc.tjdbcrow.TJDBCRowProperties) def_tDBRow_5.createRuntimeProperties();
 		                    ...
 		                    "MERGE INTO HIST.SECURITY_PROFILE_DETAILS T USING ( SELECT  DISTINCT PARTNERID, SITEID, CLIENTID, ENDPOINTID, PROFILEID, PROFILE_NAME, SCORE , Profile_Update_DateTime_NTZ AS Profile_Update_DateTime FROM SRC.SECURITY_PROFILE_RISK_HISTORY )S  ON T.ENDPOINTID=S.ENDPOINTID AND T.PROFILEID=S.PROFILEID AND T.Profile_Update_DateTime=S.Profile_Update_DateTime WHEN NOT MATCHED THEN INSERT (PARTNERID,SITEID,CLIENTID,ENDPOINTID,PROFILEID,PROFILE_NAME,SCORE,Profile_Update_DateTime) VALUES (PARTNERID,SITEID,CLIENTID,ENDPOINTID,PROFILEID,PROFILE_NAME,SCORE,Profile_Update_DateTime);");
...
org.talend.components.jdbc.tjdbcrow.TJDBCRowProperties props_tDBRow_6 =
        (org.talend.components.jdbc.tjdbcrow.TJDBCRowProperties) def_tDBRow_6.createRuntimeProperties();
 		                    props_tDBRow_6.setValue("sql",
 		                     "MERGE INTO HIST.SECURITY_CATEGORY_DETAILS T USING ( 	SELECT  		DISTINCT  		P.SPD_ID, 		H.CATEGORY_LABEL, 		H.CATEGORY_NAME, 		REPLACE(H.CATEGORY_EXECUTION_STATE,'\"','') AS CATEGORY_EXECUTION_STATE, 		REPLACE(H.CATEGORY_EXECUTION_RESULT,'\"','') AS CATEGORY_EXECUTION_RESULT, 		REPLACE(H.CATEGORY_EXECUTION_STATUS,'\"','') AS CATEGORY_EXECUTION_STATUS, 		H.CATEGORY_UPDATE_DATETIME_NTZ AS CATEGORY_UPDATE_DATETIME, 		H.REASON 	FROM  			   SRC.SECURITY_PROFILE_RISK_HISTORY H 	INNER JOIN HIST.SECURITY_PROFILE_DETAILS P  	ON  H.ENDPOINTID=P.ENDPOINTID AND  		H.PROFILEID=P.PROFILEID AND  		H.Profile_Update_DateTime_NTZ=P.Profile_Update_DateTime 		 )S ON T.SPD_ID=S.SPD_ID AND  	  LTRIM(RTRIM(UPPER(T.CATEGORY_LABEL)))=LTRIM(RTRIM(UPPER(S.CATEGORY_LABEL))) AND  	  LTRIM(RTRIM(UPPER(T.CATEGORY_EXECUTION_STATE)))=LTRIM(RTRIM(UPPER(S.CATEGORY_EXECUTION_STATE))) AND  	  T.CATEGORY_UPDATE_DATETIME=S.CATEGORY_UPDATE_DATETIME WHEN NOT MATCHED THEN INSERT (CATEGORY_LABEL,CATEGORY_NAME,CATEGORY_EXECUTION_STATE,CATEGORY_EXECUTION_RESULT,CATEGORY_EXECUTION_STATUS,CATEGORY_UPDATE_DATETIME,REASON,SPD_ID) VALUES (CATEGORY_LABEL,CATEGORY_NAME,CATEGORY_EXECUTION_STATE,CATEGORY_EXECUTION_RESULT,CATEGORY_EXECUTION_STATUS,CATEGORY_UPDATE_DATETIME,REASON,SPD_ID);");
...
org.talend.components.jdbc.tjdbcrow.TJDBCRowProperties props_tDBRow_7 =
        (org.talend.components.jdbc.tjdbcrow.TJDBCRowProperties) def_tDBRow_7.createRuntimeProperties();
 		                    ...
 		                    "MERGE INTO HIST.SECURITY_CATEGORY_REASON_DETAILS T USING ( 	Select G.SCD_ID,N.value::string as ReasonDetails            FROM ( SELECT DISTINCT  	C.SCD_ID, 	Replace(H.REASONDETAILS,'\"','') AS REASONDETAILS 	FROM  		SRC.SECURITY_PROFILE_RISK_HISTORY H 		INNER JOIN HIST.SECURITY_PROFILE_DETAILS P  			   ON  H.ENDPOINTID=P.ENDPOINTID AND  				   H.PROFILEID=P.PROFILEID AND  				   H.Profile_Update_DateTime_NTZ=P.Profile_Update_DateTime 		INNER JOIN HIST.SECURITY_CATEGORY_DETAILS C 			   ON  P.SPD_ID=C.SPD_ID AND  				   LTRIM(RTRIM(UPPER(H.CATEGORY_LABEL)))=LTRIM(RTRIM(UPPER(C.CATEGORY_LABEL))) AND  				   LTRIM(RTRIM(UPPER(REPLACE(H.CATEGORY_EXECUTION_STATE,'\"',''))))=LTRIM(RTRIM(UPPER(C.CATEGORY_EXECUTION_STATE))) AND  				   H.CATEGORY_UPDATE_DATETIME_NTZ=C.CATEGORY_UPDATE_DATETIME    ) G    ,lateral flatten(input=>split(REASONDETAILS, ',')) N ) S ON  	T.SCD_ID=S.SCD_ID AND  	LTRIM(RTRIM(UPPER(T.REASONDETAILS)))=LTRIM(RTRIM(UPPER(S.REASONDETAILS))) WHEN NOT MATCHED THEN INSERT (REASONDETAILS,SCD_ID) VALUES (S.REASONDETAILS,S.SCD_ID);");
 ...
 */
    ...
    public static void main(String[] args){
        final Load_Profiles_Risk_History_Transactional Load_Profiles_Risk_History_TransactionalClass = new Load_Profiles_Risk_History_Transactional();

        int exitCode = Load_Profiles_Risk_History_TransactionalClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Load_Profiles_Risk_History_Transactional.class.getClassLoader().getResourceAsStream("local_project/load_profiles_risk_history_transactional_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = Load_Profiles_Risk_History_Transactional.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Load_Profiles_Risk_History_Transactional");
        }
...
/************************************************************************************************
 *     233453 characters generated by Talend Open Studio for Big Data 
 *     on the June 27, 2018 12:40:55 PM IST
 ************************************************************************************************/